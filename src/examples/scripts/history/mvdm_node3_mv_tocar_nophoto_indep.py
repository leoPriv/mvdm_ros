...
class EnhancedMvdmSystem:
    def __init__(self, speed_action, lane_action):
        ...
        bridge = CvBridge()
        empty_cv_img = np.zeros((1, 1, 3), dtype=np.uint8)  # 0x0的空白RGB图像
        first_img = bridge.cv2_to_imgmsg(empty_cv_img, encoding="rgb8")
        self.current_frame = {id : [first_img,''] for id in self.robot_ids}
        self.frame_89 = [first_img,'']
        self.frame_98 = [first_img,'']
        ...
        #         Image,
        #         partial(self.sub_2_data, extra_param=robot_id),
        #         queue_size=1
        #     )
        #     for robot_id in self.robot_ids
        # }
        self.subs = {
            "9289": rospy.Subscriber(
                f'/limo9289/camera/image_raw',
                Image,
                partial(self.sub_2_data89, extra_param="9289"),
                queue_size=1
            ),
            "9298": rospy.Subscriber(
                f'/limo9298/camera/image_raw',
                Image,
                partial(self.sub_2_data98, extra_param="9298"),
                queue_size=1
            )
        }
        ...
        self.img_timer = rospy.Timer(rospy.Duration(params["control_period"]), self.preprocess_image())
        self.control_timer = rospy.Timer(rospy.Duration(params["control_period"]), self.cbControl)

    def sub_2_data89(self, image, extra_param=None):
        limo_id = extra_param
        time = datetime.now().strftime('%H:%M:%S.%f')[:-3]
        if limo_id == '9298':
            self.frame_98 = [image, time]
            img = self.bridge.imgmsg_to_cv2(self.frame_98[0], "bgr8")
            print(img.shape[:2], limo_id)
        elif limo_id == '9289':
            self.frame_89 = [image, time]
            img = self.bridge.imgmsg_to_cv2(self.frame_89[0], "bgr8")
            print(img.shape[:2], limo_id)

    def sub_2_data98(self, image, extra_param=None):
        limo_id = extra_param
        time = datetime.now().strftime('%H:%M:%S.%f')[:-3]
        if limo_id == '9298':
            self.frame_98 = [image, time]
    ...
    def preprocess_image(self):
        ...
        for limo_id in self.robot_ids:
            ...
            image = self.bridge.imgmsg_to_cv2(self.current_frame[limo_id][0], "bgr8")
