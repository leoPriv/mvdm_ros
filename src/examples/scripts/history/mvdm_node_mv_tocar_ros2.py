...
class EnhancedMvdmSystem(Node):
    def __init__(self, speed_action, lane_action):
        super().__init__('enhanced_mvdm_system')
        ...
        for robot_id in self.robot_ids:
            # 创建订阅者
            self.subs[robot_id] = self.create_subscription(
                Image,
            ...
            # 创建发布者
            self.angular_pubs[robot_id] = self.create_publisher(
                Float32,
                f'/limo{robot_id}/angular',
                10
            )

            self.vel_pubs[robot_id] = self.create_publisher(
                Float32,
                f'/limo{robot_id}/vel',
                10
            )
        ...
        # 创建定时器
        self.control_timer = self.create_timer(params["control_period"], self.cbControl)
    ...
    def preprocess_image(self, image_msg, extra_param=None):
    ...
    def publish_twist(self, limo_id, ang_pub, vel_pub, linear_x, angular_z):
        ...
        # 创建并发布消息
        vel_msg = Float32()
        vel_msg.data = float(linear_x)
        vel_pub.publish(vel_msg)

        ang_msg = Float32()
        ang_msg.data = float(angular * 1.2)
...
def main(args=None):
    ...
    try:
        rclpy.spin(mvdm_system)
    except KeyboardInterrupt:
        pass
    finally:
        mvdm_system.destroy_node()
        rclpy.shutdown()
