...
class EnhancedMvdmSystem:
    def __init__(self, speed_action, lane_action):
        ...
        self.img_timer = rospy.Timer(rospy.Duration(params["control_period"]), self.imgControl)
    ...
    def imgControl(self, event):
        for limo_id in self.robot_ids:
            self.img_pubs[limo_id].publish(1.0)
