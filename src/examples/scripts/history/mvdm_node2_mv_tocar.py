...
class EnhancedMvdmSystem:
    ...
    def calculate_deviation(self, left_line, right_line, image_width, limo_id):
        ...
        lane_center = (x_left + x_right) / 2
        # self.first_pos[limo_id] = image_width // 2 - 10 89 93
        # self.first_pos[limo_id] = image_width // 2 - 80  #98
    ...
    def publish_twist(self, limo_id,ang_pub, vel_pub, linear_x, angular_z):
        ...
        rospy.loginfo(f"publish to{limo_id} - Speed: {linear_x:.3f}, Angular: {angular:.3f}(Time: {datetime.now().strftime('%H:%M:%S.%f')[:-3]})")
...
def run():
    ...
    speed_action = {'9293': [0, 0, 0, 0, 0], "9298": [0, 0, 0, 0, 0],"9289": [0.5, 0.5, 0.5, 0.5, 0.5]}
