...
class Mvdm_System:
    def __init__(self, speed_action, lane_action):
        ...
        # self.image_sub = rospy.Subscriber('/limo9293/camera/image_raw', Image, self.image_callback)

        # self.subs = {
        #     '9293': rospy.Subscriber('/limo9293/camera/image_raw',Image,partial(self.preprocess_image, extra_param="9293")),
        #     '9298': rospy.Subscriber('/limo9298/camera/image_raw',Image,partial(self.preprocess_image, extra_param="9298")),
        #     '9289': rospy.Subscriber('/limo9289/camera/image_raw', Image,partial(self.preprocess_image, extra_param="9289"))
        # }
        self.subs = {
        ...
        # self.pubs = {
        #     '9293': rospy.Publisher('/limo9293/cmd_vel', Twist, queue_size=1),
        #     '9298': rospy.Publisher('/limo9298/cmd_vel', Twist, queue_size=1),
        #     '9289': rospy.Publisher('/limo9289/cmd_vel', Twist, queue_size=1),
        # }
        self.pubs = {
            robot_id: rospy.Publisher(
                f'/limo{robot_id}/cmd_vel',  # 动态生成话题名
                Twist,
                queue_size=1
            )
            for robot_id in self.robot_ids
    ...
    # def image_callback(self, msg,extra_param=None):
        # 开线程处理图像
        # Thread(target=self.preprocess_image, args=(msg,extra_param), daemon=True).start()
    ...
    def publish_twist(self,publisher, linear_x, angular_z):
        twist = Twist()
    ...
    def cbControl(self, event):
        ...
        for limo_id in self.robot_ids:
            if self.lane_expand[limo_id][self.count] == 0:
                ...
                self.publish_twist(self.pubs[limo_id], self.exc_speed[limo_id], self.angular[limo_id])
            else:
