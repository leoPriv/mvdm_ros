...
params = {
    ...
    "control_period": 0.1,
    "expand_num": 40,
    "control_angular": 0.075,  # 降低控制角速度
    "control_angular_max": 0.12,
    "control_angular_min": 0.05,
...
class Mvdm_System:
    ...
    def init_camera(self):
        ...
        try:
            ...
            if self.cap.isOpened():
                ...
                #shutter_ms = 10.0  # 10 ms
                #shutter_us = int(shutter_ms * 1000)  # 转换为微秒
                #self.cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, 0.25)  # 手动模式
                #exposure_value = -int(shutter_us / 100)  # OpenCV曝光值转换
    ...
    def enhanced_pid_control(self, deviation):
        ...
        if current_speed > 0:
            # 速度越快，控制增益越大
            speed_factor = min(current_speed / current_speed, 2.0)  # 最大2倍增益
    ...
    def preprocess_image(self, image):
        ...
        if self.flag == 1:
            # save image
            image_path = os.path.join(os.getcwd(),f"picture.jpg")
            cv2.imwrite(image_path,lower_half)
        ...
        # 应用黄色掩码
        masked_image = cv2.bitwise_and(enhanced, enhanced, mask=yellow_mask)
        ...
        if self.flag == 1:
            # save image
            image_path = os.path.join(os.getcwd(),f"pictureedge.jpg")
        ...
        if self.flag == 1:
            # save image
            image_path = os.path.join(os.getcwd(),f"picturel.jpg")
        ...
        rospy.loginfo(f"Deviation: {self.filtered_deviation:.2f}, Angular: {self.angular:.3f}")
    ...
    def publish_keep_twist(self, linear_x, angular_z):
        ...
        if abs(angular_z) > params["control_angular_max"]:
            ...
            # 按比例缩放，而不是直接置零
            angular = angular_z * (params["control_angular"] / params["control_angular_max"]) / 2
            if abs(angular) < params["control_angular_min"]:
                if angular > 0:
                    angular = params["control_angular_min"]
                else:
                    angular = -params["control_angular_min"]
        limo.SetMotionCommand(linear_x, -0.01, 0, angular)

        rospy.loginfo(f"SetkpCommand - Speed: {linear_x:.3f}, Angular: {angular:.3f}")
    ...
    def publish_lc_twist(self, linear_x, angular_z):
        ...
        rospy.loginfo(f"SetlcCommand - Speed: {linear_x:.3f}, Angular: {angular_z:.3f}")
    ...
    def cbControl(self, event):
        ...
        if self.exc_lane == 0:
            ...
            progress = min(self.index_lane / 20, 1.0)
            phase = progress * math.pi * 2
            phase_allo = 0
            if phase <= math.pi:
                angular = self.exc_lane * math.sin(phase) * (1 - phase_allo)
            else:
                angular = self.exc_lane * math.sin(phase) * (1 + phase_allo)
            ...
            try:
                ...
                speed_compensation = 1 / math.cos(abs(current_angular))
            ...
            self.index_lane += 1
            if self.exc_lane == -1:
                exc_angular = current_angular * (1 / actual_speed) / 5.5
            else:
                exc_angular = current_angular * (1 / actual_speed) / 6.5
